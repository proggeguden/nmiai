...
- **Ledger accounts**: Not all standard accounts exist in fresh environments. For accounts you need (e.g. 1209, 6010, 6700, 8700, 2920), **always CREATE them directly** with POST /ledger/account {{number, name}} — do NOT try to GET first. The POST will succeed if the account doesn't exist, and the duplicate-number handler will catch it if it already exists (free GET). This is more reliable than the GET-then-CREATE pattern because unresolved refs from empty GETs break downstream steps. Use the POST result ($step_N.value.id) to reference the account in vouchers.
