...
- **Employment details**: POST /employee/employment/details — use **percentageOfFullTimeEquivalent** (NOT employmentPercentage). occupationCode must be {{"id": <int>}} not bare string. To look up occupation codes: GET /employee/employment/occupationCode (NOT /salary/occupationCode).
...
[
  ...
  {{"step_number": 3, "tool_name": "call_api", "args": {{"method": "POST", "path": "/product", "body": {{"name": "Consulting", "number": "5001", "priceExcludingVatCurrency": 25000.0, "vatType": {{"id": 3}}}}}}, "description": "Create product if not found"}},
  {{"step_number": 4, "tool_name": "call_api", "args": {{"method": "POST", "path": "/order", "body": {{"customer": {{"id": "$step_1.value.id"}}, "orderDate": "{today}", "deliveryDate": "{today}", "orderLines": [{{"product": {{"id": "$step_2.values.length > 0 ? $step_2.values[0].id : $step_3.value.id"}}, "description": "Consulting", "count": 1, "unitPriceExcludingVatCurrency": 25000.0}}]}}}}, "description": "Create order — product ref uses ternary: GET result if found, else POST result"}},
  {{"step_number": 5, "tool_name": "call_api", "args": {{"method": "PUT", "path": "/order/$step_4.value.id/:invoice", "query_params": {{"invoiceDate": "{today}"}}}}, "description": "Convert order to invoice (do NOT include paidAmount here)"}},
  {{"step_number": 6, "tool_name": "call_api", "args": {{"method": "GET", "path": "/invoice/paymentType", "query_params": {{"count": 1, "fields": "id"}}}}, "description": "Get payment type (NEVER hardcode paymentTypeId)"}},
  {{"step_number": 7, "tool_name": "call_api", "args": {{"method": "PUT", "path": "/invoice/$step_5.value.id/:payment", "query_params": {{"paymentDate": "{today}", "paymentTypeId": "$step_6.values[0].id", "paidAmount": "$step_5.value.amount", "paidAmountCurrency": "$step_5.value.amount"}}}}, "description": "Register full payment using real invoice amount"}}
]
```
