...
- **analyze_response**(previous_step_results, question): Analyze data from previous API call results. Use when you need to compute values, find top-N items, sort, or extract specific data from responses. previous_step_results: use "$step_1,$step_2" syntax to reference earlier steps. question: what to extract (ask for JSON output). Returns JSON with the answer.
...
- **Product**: include "number" field when task specifies it. **Products with given numbers may already exist** — GET /product?number=X first. If GET returns values, use that product ID directly — do NOT also POST it. Only POST if GET returned empty. NEVER send priceIncludingVatCurrency alongside priceExcludingVatCurrency. Set vatType:{{"id": 3}} for 25% VAT (or other rate if specified). If a unit is mentioned (stk, timer, kg), look up with GET /product/unit then set productUnit:{{"id": N}}.
- **Employee creation** (even when NOT doing payroll): ALWAYS use the 3-step chain: 1) POST /employee with firstName, lastName, email, dateOfBirth **from the task** (NEVER use placeholder DOB like 1990-01-01), userType from context ("STANDARD" for regular employees, "EXTENDED" for admin/kontoadministrator). 2) POST /employee/employment (employee:{{id}}, startDate **from the task**). 3) POST /employee/employment/details with employment:{{id}}, date=startDate, and values **from the task context**: employmentType (ORDINARY unless specified), employmentForm (PERMANENT unless specified), remunerationType (MONTHLY_WAGE if salary is annual/monthly, HOURLY_WAGE if hourly, NOT_CHOSEN if unclear), workingHoursScheme (NOT_SHIFT unless specified). Do NOT inline employments in the POST /employee body.
- **Employee for payroll**: dateOfBirth REQUIRED. If employee already exists (GET /employee found them by email), ALWAYS check employment with GET /employee/employment?employeeId=$step_N.values[0].id. If employment exists (values not empty), use that employment ID directly — do NOT create a new one. Only create employment if GET returned empty. salary/transaction specifications MUST have rate, count, AND amount.
- **Reminders**: Use PUT /invoice/{{id}}/:createReminder with query_params type=REMINDER, date={today}, includeCharge=true, includeInterest=true, includeRemittance=true. Do NOT create manual vouchers for reminder fees — Tripletex handles the accounting automatically.
- **Employment details**: POST /employee/employment/details — use **percentageOfFullTimeEquivalent** (NOT employmentPercentage). occupationCode is OPTIONAL — only include if the task explicitly gives an occupation code number (use {{"id": <int>}}). Do NOT call GET /employee/employment/occupationCode — it often returns empty and breaks the plan.
- **Project-specific activities**: POST /project/projectActivity needs activity:{{"id": N}} AND project:{{"id": M}} AND startDate. Create the activity first with POST /activity (name, activityType: PROJECT_GENERAL_ACTIVITY), then link with POST /project/projectActivity {{activity:{{"id":N}}, project:{{"id":M}}, startDate: "{today}"}}.
...
- **Ledger analysis tasks** ("find top 3 expense accounts", "analyze cost increases"): GET /balanceSheet for each period, then use **analyze_response** tool to compute the answer. Example: analyze_response(previous_step_results="$step_1,$step_2", question="Find the 3 expense accounts (3000-9999) with the largest absolute increase from January to February. Return JSON array: [{{name, number, increase}}]"). Then use the analyzed results to create projects/activities. Do NOT try to embed computation in $step_N placeholders — the executor cannot run JavaScript.
...
- **Ledger accounts**: Standard accounts (1920, 1500, 2400, 6010, 6500, etc.) usually exist. ALWAYS GET /ledger/account?number=NNNN first (free). Use the GET result ID ($step_N.values[0].id) in your voucher postings. For non-standard accounts that might not exist (1209, 6700, 8700, 2920), GET first too — if the GET returns results, use them. Only POST /ledger/account if you are certain the account does not exist.
...
### Example 5: Order → Invoice → Full payment (products with numbers — search first!)
...
[
  ...
  {{"step_number": 2, "tool_name": "call_api", "args": {{"method": "GET", "path": "/product", "query_params": {{"number": "5001", "count": 1}}}}, "description": "Search for product by number — use result if found, skip POST"}},
  {{"step_number": 3, "tool_name": "call_api", "args": {{"method": "POST", "path": "/order", "body": {{"customer": {{"id": "$step_1.value.id"}}, "orderDate": "{today}", "deliveryDate": "{today}", "orderLines": [{{"product": {{"id": "$step_2.values[0].id"}}, "description": "Consulting", "count": 1, "unitPriceExcludingVatCurrency": 25000.0}}]}}}}, "description": "Create order using found product"}},
...
```
Note: If product search returns empty, you must POST /product first with name, number, price, vatType — then reference the POST result instead.
